...
const { listQuotations, upsertQuotation, deleteQuotationById } = require("../../lib/quotations-store");
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const result = await listQuotations({ tenantId, page, limit, search });
  ...
  if (req.method === "POST") {
    try {
      ...
      const item = await upsertQuotation(tenantId, payload);
