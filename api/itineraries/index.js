...
const { listItineraries, upsertItinerary, deleteItineraryById } = require("../../lib/itineraries-store");
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const result = await listItineraries({ tenantId, page, limit, search });
  ...
  if (req.method === "POST") {
    try {
      ...
      const item = await upsertItinerary(tenantId, payload);
  ...
  if (req.method === "DELETE") {
    try {
      const id = getId(req);
      if (!id) return json(req, res, 400, { ok: false, error: "Missing itinerary id" });
      const deleted = await deleteItineraryById(tenantId, id);
      if (!deleted) return json(req, res, 404, { ok: false, error: "Itinerary not found", deleted: false });
