...
async function verifyCredentialsAnyTenant(usernameRaw, passwordRaw, preferredTenantId = "") {
  const username = String(usernameRaw || "").trim().toLowerCase();
  const password = String(passwordRaw || "");
  if (!username || !password) return { ok: false, message: "Usuario y contraseña son obligatorios." };

  const firstTenant = String(preferredTenantId || "").trim();
  const collection = await getAuthCollection();

  if (firstTenant) {
    const preferredExists = await collection.findOne({ tenantId: firstTenant }, { projection: { tenantId: 1 } });
    if (preferredExists) {
      const direct = await verifyCredentials(firstTenant, username, password);
      if (direct.ok) return { ...direct, tenantId: firstTenant };
    }
  }

  const candidates = await collection.find({ "users.username": username }).project({ tenantId: 1 }).toArray();
  const matches = [];
  for (const row of candidates) {
    const tenantId = String(row?.tenantId || "").trim();
    if (!tenantId || tenantId === firstTenant) continue;
    const checked = await verifyCredentials(tenantId, username, password);
    if (checked.ok) matches.push({ ...checked, tenantId });
  }
  ...
  if (matches.length > 1) {
    return { ok: false, message: "Cuenta ambigua. Usa el tenant correcto para iniciar sesión." };
  }
